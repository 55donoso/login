...
function eventListeners() {
    equipoInput.addEventListener('change', datosCita);
...
//obgeto con la informasion de la cita
const citaObj = {
    equipo: '',
    propietario: '',
    telefono: '',
...
//agregar datos al obgeto de cita
...
class UI {
   ...
   imprimirCitas({citas}) { // Se puede aplicar destructuring desde la función...
        ...
        citas.forEach(cita => {
            const {equipo, propietario, telefono, fecha, hora, destino, id } = cita;
            ...
            const equipoParrafo = document.createElement('h2');
            equipoParrafo.classList.add('card-title', 'font-weight-bolder');
            equipoParrafo.innerHTML = `${equipo}`;
            ...
            // Agregar al HTML
            divCita.appendChild(equipoParrafo);
...
function nuevaCita(e) {
    ...
    const {equipo, propietario, telefono, fecha, hora, destino } = citaObj;

    // Validar
    if( equipo === '' || propietario === '' || telefono === '' || fecha === ''  || hora === '' || destino === '' ) {
